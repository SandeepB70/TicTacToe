'''
Represents a basic tic tac toe game meant to be played through the terminal
'''
...
player1 = 'X'
player2 = 'O'
# Tells the game who is currently making their move according to the number 1 or 2. 
# 1 is for player1 and 2 is for player2
crnt_player = 1
...
def check3(player_num):
    '''
    Checks for three in a row for the current player. *player_num* represents which player we are checking
    three in a row for.

    Return:
        True if the player has three in a row, otherwise False.
    '''
...
for x in range(0,9):
    ...
    while invalid:
        try:
            ...
            print("Please enter a number from the game board.")
            continue
        if player_move < 1 or player_move > 9:
            print("Please select a valid position on the board.")
            continue
        ...
        if len(moves_made) != 0:
            if player_move in moves_made:
                print("Position has already been taken, please select another.")
                continue
        if crnt_player == 1:
